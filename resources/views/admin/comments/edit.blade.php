{{--{{$comment->user->name}}: <span>{{($comment->body)}}</span>
    @if($comment->user->id == Auth::user()->id)
        <a href="{{URL::to('/admin/comments/edit/'.$comment->id)}}" class="editComment">save</a> |
        <a href="{{URL::to('/admin/comments/delete/'.$comment->id)}}">Delete</a>
    @endif
    <div id="text{{$comment->id}}" class="edit-comment">--}}
<form id="editComment" action="{{URL::to('/admin/comments/edit/'.$comment->id)}}" method="post">
    <input type="hidden" name="_token" value="{{csrf_token()}}"/>
    <input type="hidden" name="id" value="{{$comment->id}}"/>
    <input type="hidden" name="task_id" value="{{$comment->task->id}}"/>
    <input type="hidden" name="user_id" value="{{$comment->user->id}}"/>
    <input name="body" class="form-control text-comment" cols="50" rows="5" type="text" value="{{$comment->body}}">
    <span id="notifica">Enter to save or Esc to Exit</span>
</form>
    {{--</div>--}}
